...
for day in range(7):
    ...
    for company in companies:
        for hour in range(9, 22):  # 9 AM to 9 PM
            ...
            # Promised vs Actual Delivery Logic
            promised_time = 10
            actual_time = random.randint(7, 14)
            on_time = "Yes" if actual_time <= promised_time else "No"
            early_by = max(0, promised_time - actual_time)
            delay_by = max(0, actual_time - promised_time)
            ...
            data.append({
                ...
                "Promised Time (min)": promised_time,
                "Actual Time (min)": actual_time,
                "Delivered On Time": on_time,
                "Early By (min)": early_by,
                "Delay By (min)": delay_by
            })

df = pd.DataFrame(data)
columns_to_null = ['Traffic Level', 'Temperature (°C)', 'Rain', 'Holiday', 'Orders', 'Actual Time (min)']
for col in columns_to_null:
    df.loc[df.sample(frac=0.05, random_state=42).index, col] = np.nan

df.to_excel("finaltwo_delivery_dataset_with_promised_actual_and_nulls.xlsx", index=False)
print(" Dataset saved as 'final_delivery_dataset_with_promised_actual_and_nulls.xlsx'")
